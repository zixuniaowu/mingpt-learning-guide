# -*- coding: utf-8 -*-
"""小红书「图解特辑」v3：是什么 → 为什么重要 → 对用大模型有什么用 → 6 张关键图 → CTA。
1080x1920 竖版, 男声(云健), ~75s。"""
import os
import subprocess
import sys
...
FIGURES = [
    (31, "第一张：GPT 的完整数据流。从 token 到 logits，一条线贯穿全书。"),
...
NARR = [
    ...
    "这本《从第一性原理理解 GPT》，就是回答这个问题的。它基于 Karpathy 的 minGPT：111 页、五大部分三十二章、60 多张手绘图解，源码全部可以亲手运行。",
    "为什么这本书重要？因为提示词会过时，工具会淘汰，但大模型的底层原理，八年了没变过。会用 AI 的人很多，懂 AI 的人稀缺。",
    "学会它能干什么？用得明白——知道幻觉从哪来，就知道什么能信；用得好——懂了采样和温度，参数才会调；往上走——Agent、微调、RAG，地基全是它。",
    "而书里最值钱的，就是这些图。我挑六张，带你看看。",
    FIGURES[0][1],
    FIGURES[1][1],
    FIGURES[2][1],
    FIGURES[3][1],
...
def main():
    ...
    deck = render_deck()
    visuals = deck[0:5] + fig_imgs + [deck[5]]
    assert len(visuals) == len(NARR) == 12
    ...
    for i, (img, text) in enumerate(zip(visuals, NARR), 1):
        ...
        mp3 = os.path.join(WORK, f"n{i}_{VOICE}_{key}.mp3")
        if not (os.path.exists(mp3) and os.path.getsize(mp3) > 0):
            tts(text, mp3, voice=VOICE)
        seg = os.path.join(seg_dir, f"s{i}.mp4")
        d = segment(img, mp3, seg, zoom=(6 <= i <= 11))
        segs.append(seg)
        total += d
        print(f"  seg {i}/12  tts {d - 1.0:5.1f}s")
